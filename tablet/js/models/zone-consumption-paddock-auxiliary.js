(function() {
  ...
  var model = Application.modelFactory('zone-consumption-paddock-auxiliary')
  ...
  model.model_update = function (resolve, reject) {
    $.get(Application.Env.get('backend_url')+'/zoneenergyconsumption/paddock/pit_utilities')
      .done(function(result) {
        var total = Application.models.ZoneConsumptionPaddock.data.value
        console.info("Updating zone-consumption-paddock-auxiliary: "+result.time, result)
        var percentage = Math.round((result.energy/total)*100)
        $('#donut_histogram .bar_container [data-percentage="auxiliary"]').css('width',percentage+"%");
  ...
  Application.models.ZoneConsumptionPaddockAuxiliary = model
