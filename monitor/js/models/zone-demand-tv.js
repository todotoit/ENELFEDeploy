(function() {
  ...
  var model = Application.modelFactory('zone-demand-tv')
  ...
  model.model_update = function (resolve, reject) {
    $.get(Application.Env.get('backend_url')+'/zonepowerdemand/tv_compound')
      .done(function(result) {
        var total = Application.models.InstantPowerConsumption.data.current
        console.info("Updating zone-demand-tv: "+result.time, result)
        var percentage = Math.round((result.power/total)*100)
        $('#microgrid .bar_wrap [data-percentage="tv"]').css('height', percentage+"%");
  ...
  Application.models.ZoneDemandTv = model
