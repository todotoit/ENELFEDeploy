(function() {
  ...
  var model = Application.modelFactory('zone-demand-paddock-charging')
  ...
  model.model_update = function (resolve, reject) {
    $.get(Application.Env.get('backend_url')+'/zonepowerdemand/paddock/car_charging')
      .done(function(result) {
        var total = Application.models.ZoneDemandPaddock.data.value
        console.info("Updating zone-demand-paddock-charging: "+result.time, result)
        // var percentage = Math.round((result.power/total)*100)
        // $('#donut_histogram .bar [data-percentage="charging"]').css('width',percentage+"%");
  ...
  Application.models.ZoneDemandPaddockCharging = model
